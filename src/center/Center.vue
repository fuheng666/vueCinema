<template>
   <div class="center">
      <header class="Personal">
          <div class="PersonalTop">
              <img src="https://ss2.bdstatic.com/70cFvnSh_Q1YnxGkpoWK1HF6hhy/it/u=3649178992,1821853682&fm=26&gp=0.jpg" alt="">
            <span>{{newname}}</span>
          </div>
      </header>
        <main>
              <div class="ORDER">
              <ul>
                  <li><img src="data:image/png;base64," alt="data:image/png;base64,"><span>电影订单</span></li>
                  <li><img src="data:image/png;base64," alt=""><span>商品订单</span></li>
              </ul>
          </div>
        </main>
        <section></section>
        <footer>
            <ul>
                <li><img src="data:image/png;base64," alt=""><p>卖座卷</p><img src="data:image/png;base64," alt="data:image/png;base64,"></li>
                <li><img src="data:image/png;base64," alt=""><p>组合红包</p><img src="data:image/png;base64," alt="data:image/png;base64,"></li>
                <li><img src="data:image/png;base64," alt=""><p>余额</p><img src="data:image/png;base64," alt=""></li>
                <li><img src="data:image/png;base64," alt=""><p>设置</p><img src="data:image/png;base64," alt=""></li>
            </ul>
        </footer>
   </div>
</template>

<script>
import { mapState } from 'vuex'
import { Toast } from 'vant'
export default {
  data () {
    return {
      newname: localStorage.getItem("username")
    }
  },
  computed: {
    ...mapState('loginModule', ['name'])
  },
  beforeMount () {
      console.log(this.newname);
       Toast.clear()
  }
}
</script>
<style lang="scss" scoped>
header{
      display: flex;
      align-items: center;
    height: 2rem;
    background: url(https://assets.maizuo.com/h5/v5/public/app/img/bg.6837f67.png);
    background-size: cover;
    .PersonalTop{
        width: 100%;
        padding-left:.15rem ;
        display: flex;
        align-items: center;
        img{
            width:.7rem;
            border-radius:50% ;
            overflow: hidden;
        }
        span{
            margin-left: .2rem;
            color: #fff;
            font-size: .16rem;
        }
    }

}
  .ORDER{
      padding: .15rem;
        ul {
           display: flex;
           justify-content: space-between;
           align-items: center;
           li{
                display: flex;
                flex-direction: column;

                padding: 0 .3rem;
                img{
                    width: .26rem;
                    margin-left:.12rem ;
                }
                span{
                    color: #797d82;
                    font-size:.13rem ;
                }
           }
        }
    }
    footer{
        ul li{
            display: flex;
            align-items: center;
            padding: .1rem;
            img:first-child{
                width: .2rem;
            }
            p{
                width: 3rem;
                margin-left: .1rem;
            }
             img:last-child{
                 width: .08rem;
             }
        }
    }
    section{
        height: .11rem;
        background: #F4F4F4;
    }
</style>